...
def generateFlowchart(recipe, colorMap={"preProcess": "gold", "postProcess": "darkcyan", "analysis": "forestgreen", "postAnalysis": "firebrick"}):
    ...
    networkx.draw_networkx(G, pos, with_labels=True, node_size=1)
    for level in levelKey:
        ...
        networkx.draw_networkx(G.subgraph(sGnodes), pos, with_labels=True,font_size=20, bbox=dict(facecolor=colorMap[levelKey[level]], edgecolor="black", boxstyle="round,pad=0.2"))
    networkx.draw_networkx_edges(G, pos, edge_labels, connectionstyle="arc3,rad=0.35")
